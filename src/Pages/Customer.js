...
class Customer extends React.Component {
    ...
    // getCustomers -> untuk mengakses API get customer
    getCustomers = () => {
    ...
    Edit = selectionItem => {
        ...
        this.setState({
            action: "update",
            customer_id: selectionItem.customer_id,
            name: selectionItem.name,
            phone: selectionItem.phone,
            address: selectionItem.address,
            image: null,
    ...
    // function saveCustomer -> untuk menyimpan data pada db dengan mngakses API
    saveCustomer = event => {
    ...
    // fuction dropCustomer -> digunakan untuk menghapus data
    dropCustomer = selectionItem => {
        if (window.confirm("are you sure will delete this item")) {
            let url = base_url + "/customer/" + selectionItem.customer_id
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <form onSubmit={ev => this.saveCustomer(ev)}>
                                    ...
                                    Customer Phone
                                    <input type="text" className="form-control mb-1" value={this.state.phone}
                                    onChange={ev => this.setState({phone: ev.target.value})} required />

                                    Customer Address
                                    <input type="text" className="form-control mb-1" value={this.state.address}
                                    onChange={ev => this.setState({address: ev.target.value})} required />
